...
const META = {
  ...
  ble_cts_time: ['读', '{field, charUuid, bytes, hex, utc, skewSecs, year, month, day, hour, minute, second, dayOfWeek, dayOfWeekName, fractions256, fractionMillis, adjustReason, adjustReasons, notes}', '把 CTS（Current Time Service 0x1805）的原始值翻译成人话 —— **纯后端工具**：不碰设备也不碰界面（读值仍走 `ble_read` → `ble_get_output`）。`data` 给 `2A2B` 的 10 字节或 `2A0F` 的 2 字节都认；返回里 `utc`/`skewSecs` 是结论，`notes` 会主动指出可疑处（年份像 RTC 没初始化、星期与日期对不上、时钟偏了多少分钟）。'],
