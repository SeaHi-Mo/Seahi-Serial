...
const META = {
  ...
  ble_cts_time: ['读', '{field, charUuid, bytes, hex, utc, skewSecs, year, month, day, hour, minute, second, dayOfWeek, dayOfWeekName, fractions256, fractionMillis, adjustReason, adjustReasons, notes}；`field:"localTimeInfo"`（2 字节）时是 `{timeZoneQuarterHours, utcOffset, utcOffsetMinutes, dstOffset, dstName, dstOffsetMinutes, notes}`', '把 CTS（Current Time Service 0x1805）的原始值翻译成人话 —— **纯后端工具**：不碰设备也不碰界面（读值仍走 `ble_read` → `ble_get_output`）。`data` 给 `2A2B` 的 10 字节或 `2A0F` 的 2 字节都认；返回里 `utc`/`skewSecs` 是结论，`notes` 会主动指出可疑处（年份像 RTC 没初始化、星期与日期对不上、时钟偏了多少分钟）。⚠️ **设备没给出的字段回 `null`，不许猜一个具体值**：时区 `-128` → `utcOffset:null`、DST `0xFF` → `dstOffsetMinutes:null`（`timeZoneQuarterHours`/`dstOffset` 保留原始字节）。DST 的值是 **15 分钟单位**：2=+0.5h / 4=+1h / 8=+2h；越界与保留值都会写进 `notes`。'],
