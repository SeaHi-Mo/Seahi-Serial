...
const META = {
  ...
  serial_quick_cmd: ['读', '{pane, items:[{index,label,value}], usable, file, source}', '不带 index 只列；带 index 才执行（→ {pane, ran, label, value}）。`source=file` 表示这个列表来自外部文件（面板里增删改会写回该文件），`file` 是它的路径；`source=config` 才是纯配置里的列表'],
