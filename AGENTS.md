...
前端**有**无头断言集 `.walkthrough/gen_ble_preview.js`（当前 1357 条，随代码演进增补；MCP 的 npm 安装器另有
...
11. **每个工具都必须有"返回值契约"和"调用情况"测试**（用户的要求："不然预期的结果怎么确定
    是否已经完成？"）。三条一起才叫测过：
    ① **返回值契约**（`every_tool_has_a_tested_return_contract`）：44 个工具每个都要在表里交代
...
- `doc/MCP.md` — MCP 使用说明（面向使用者）｜`doc/MCP_TOOLS.md` — **44 个工具的参考手册**（工具名/描述/入参由 `.walkthrough/gen_mcp_tools_doc.js` 从 `protocol.rs` 生成，返回结构是实调抓的）｜`doc/MCP_DESIGN.md` — MCP 设计文档（含每步的实施记录）
