...
- `src/index.html` — 整个前端（单文件，VS Code Dark 主题风格）
- `src-tauri/src/main.rs` — 整个 Rust 后端（约 660 行）：串口枚举（SetupAPI）、多串口连接/断开、DTR/RTS 切换、收发数据
- `src-tauri/Cargo.toml` — Rust 依赖（serialport 3.3, rfd 0.15, winapi 0.3, reqwest 0.11）
