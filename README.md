...
监控输出区最右侧那条窄条就是快速指令分栏（默认折叠，点一下展开，拖动可调宽）。每条指令一行六格：

| 格子 | 说明 |
|------|------|
| 顺序号 | 方形小框，默认 `0`。`0` = 不参与循环发送；大于 `0` 的按数字**从小到大**依次发送（参与的那几条会点亮） |
| 指令内容 | 要发的内容，回车即发（文本模式；转义符如 `\r\n` 生效） |
| 延时 | 毫秒，默认 `1000`：本条发完到下发一条的间隔 |
| HEX | 本条的 HEX 使能，默认关闭。开启后本条按 HEX 解析发送（与主发送栏的文本/HEX 无关，可一条文本一条 HEX 混着跑） |
| 发送 / 删除 | 单独发这一条 / 删掉这一条 |

标题行左侧的 **「循环发送」** 开关点一下打开、再点一下关闭：按顺序号依次发送，一轮发完从头再来。
没连串口、或没有任何顺序号大于 0 的指令时会**拒绝开启**并说明原因；跑的过程中掉线、关掉分栏、
或列表里再没有可发的指令，都会**自动停止**并提示。把分栏折叠起来循环**不会停**，折叠条底部会
点一颗一闪一闪的小点提醒你（提示里也写着「循环发送进行中，展开可停止」）。
（循环发送的开关状态不随配置保存 —— 开机自动发指令太危险。）

指令列表还可以**挂到一个外部文件**上（标题行的「导入」）：挂上之后增删改都写回那个文件，
`#` 注释、表头、额外列都原样保留。**表格的表头写了哪几列就管哪几列**：
`| 名称 | 指令 | 顺序号 | 延时(ms) | HEX |` —— 没写这三列的文件按老规矩读（第 3 列起是你的备注，
原样保留，绝不会被当成顺序号改写）。「导出」得到的是一份**自包含副本**：一律带上这三列
（纯指令行文件放不下时会升级成 Markdown 表格），拿它换台机器导入，循环配置一项不丢。
