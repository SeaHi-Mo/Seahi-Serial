...
| 只读（沙箱）模式 | **关**（默认）。打开后所有写操作被拒（`-32007`，未执行）；弹窗里一键开关 |
...
| D6 | **Agent 跑在沙箱/容器里时怎么连上我们** | 我们**只监听回环**（安全要求，不许改）。容器内的 `127.0.0.1:7777` 不是宿主机 → 每次调用都 connection refused，如果客户端就在容器里，**它永远连不上**。三个选项：① **客户端跑在宿主机**（推荐，最省事、也最安全）；② 做 **stdio 适配器**（曾明确"不做"，B 方案）——但适配器也必须跑在宿主机才连得上，没解决容器问题；③ 只在文档里写清"容器需端口转发/同宿主网络"。**要你定** |
...
| commit | 内容 |
|---|---|
| `be70822` | 按官方 MCP Inspector 的 portability 检查修 `ui_set` 的 schema（空 `{}` → 带类型 → 最终用 `anyOf`），并固化成不变式 |
| `ba48a37` | README 体现 TODO 与最新状态；修掉 `RELEASE_NOTES` 里写反的"默认不启动" |
| `aad8b7a` | `TODO.md` 按实测重写并**纳入版本库** |
...
**碰任何东西之前**被拒（`-32007`，消息写明"没有执行"），界面与配置一个字不变；`mcp_status.readOnly`
