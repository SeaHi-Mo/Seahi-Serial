...
| `.qcmd-cols` | **列标题行**（顺序 / 指令 / 延时 / HEX）：住在 `.qcmd-list` **里面**（`position:sticky; top:0` + 不透明底），与 `.qcmd-item` **同一套 `grid-template-columns`/`areas`** 所以永远对齐。放里面是必须的：列表一出滚动条就占 8px 宽，**列表外**的元素会比数据行宽 8px，两边的分隔线差一截（用户 2026-09 报的"每行下方的线不够长"；实测溢出时行宽 278px，放外面时列标题是 286px）。`rebuildQcmdList` 清空列表后要用 `qcmdColsHtml(mid)` 把它补回来 | grid + sticky |
...
- 宽度可调：**展开后**拖动折叠条即可调宽（往左拖变宽，`cursor:col-resize`）；折叠态不启用拖动（那一下的语义是"展开"）
  - 宽度走 CSS 变量 `--qcmd-side-w`（缺省 300px：一条六格要放得下），钳在 120–640px，且永远给输出区留 160px
  - 按监视器各记一份、写入 `config.json`（`qcmdSideWidth`），重启/切页后沿用；拖动位移 < 3px 仍按点击处理（松手那次 click 不会把刚调好的分栏收起来）
- 标题行（不再有「快速指令」文字）只有控件：**`循环发送` 开关** + `＋ 添加` / `导入` / `导出`，
  **靠左聚拢**（`justify-content:flex-start`）：原来的 `space-between` 是"左一个开关、右三个按钮、
  中间一大段空"，既散又违背"开关在＋添加左侧"的原始要求；靠左后还与数据行第一列对齐成一条轴
- **列标题行**（`.qcmd-cols`：顺序 / 指令 / 延时 / HEX）压在列表上方，与数据行同一套 grid 轨道 ——
  宽栏下"一排飘着的框"因此读得成一张表（这是"排版不够好"的另一半）。
