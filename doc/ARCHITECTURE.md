...
| `.qcmd-cols` | **列标题行**（顺序 / 指令 / 延时(ms) / HEX）：住在 `.qcmd-list` **里面**（`position:sticky; top:0` + 不透明底），与 `.qcmd-item` **同一套 `grid-template-columns`/`areas`** 所以永远对齐；六列**全是固定宽**（延时 48px / HEX 34px）—— `auto` 是每个 grid 各按自己内容算的（表头里是文字、行里是按钮），列边界会不一致。单元格的 padding 照抄对应输入框（val 左 5px、delay 右 3px），字才真的对着格。放里面是必须的：列表一出滚动条就占 8px 宽，**列表外**的元素会比数据行宽 8px，两边的分隔线差一截（用户 2026-09 报的"每行下方的线不够长"；实测溢出时行宽 278px，放外面时列标题是 286px）。`rebuildQcmdList` 清空列表后要用 `qcmdColsHtml(mid)` 把它补回来 | grid + sticky |
...
- 标题行（不再有「快速指令」文字）只有控件，**分两端**：左 = 状态（`循环发送` 开关）、
  右 = 动作（`＋ 添加` / `导入` / `导出`，包在 `.qcmd-hd-acts` 里）——
  常规工具条的分法，用户 2026-09 明确要求"＋添加/导入/导出 还是摆右侧"
  （中途试过整体靠左聚拢，用户否了）
- **列标题行**（`.qcmd-cols`：顺序 / 指令 / 延时(ms) / HEX）压在列表上方，与数据行同一套 grid 轨道，
  六列全是固定宽、单元格 padding 照抄输入框 —— 宽栏下"一排飘着的框"因此读得成一张表（这是"排版不够好"的另一半）。
  ⚠️ 它在 `.qcmd-list` **里面**（sticky 钉顶）：列表出滚动条时滚动条占 8px 宽，
  放外面的话列标题的线会比每行下方的线长 8px（用户报的"线不够长"）
  （开关在「＋ 添加」左侧，点一下开、再点一下关；开启态填主题色 + 亮圆点）
- 每条指令是**一行六格**：`顺序号 | 指令内容 | 延时(ms) | HEX | 发送 | 删除`
  - **顺序号**（方形框，默认 `0`）：`0` = 不参与循环发送；`>0` 参与，按数字**从小到大**依次发（同号按列表先后）。参与的那几条描边点亮
  ...
  - 三个新控件都带 id（`{mid}-qcmdi-{i}-seq/-delay/-hex`），否则 MCP 控件注册表只能靠文档序兜底
...
- **面板里的横向分隔线**（标题行 / 来源行 / 列标题 / 每行下方）**必须等长，且左端顶到面板左缘** ——
  两条都是像素量出来的：
  - **等长**：列表 `overflow-y:auto` 出滚动条时占 8px 宽，滚动容器**外面**的元素就比数据行宽 8px，
    ...
    ② 标题行/来源行 `margin-right:8px` 让出同一条槽；③ 列标题住在 `.qcmd-list` **里面**（sticky）。
  ...
  - **左端顶到左缘**：展开态把折叠条**脱离文档流**（`position:absolute; left:0`），面板内容因此拿到
    ...
    见 `.qcmd-item` / `.qcmd-cols` / `.qcmd-side-hd` / `.qcmd-side-src`。
    折叠条 `z-index:3` 压在 sticky 列标题（`z-index:1`）之上，色条才不会被列标题的不透明底盖断
...
- **顺序号 / 延时 / HEX 三列：表头驱动**（2026-09）。文件表头写了列名，那一列才被当作该参数：
  ...
  - **写回挂载文件不擅自补列**（用户的表结构由用户定）；表头声明了列却缺列的条目，
    新增数据行会按表头的列补齐（`0 / 1000 / text`），不会把表格撑歪。
  - **没动过的格子原样回吐**：用户写的 `0` / `是` / 空格原样保留，只有他真改了值（或面板新建/导出）
    才规范化成 `0 / 1000 / text`（`qcmdParamCell`）—— 往返保真因此仍成立。
  ...
  - 顺带：名称已退出界面，**名称与内容都空的条目不写回文件**（写进去也活不过一次重载 —— 解析端把空行当结构行丢掉）。
- **导出 = 自包含快照**：`qcmdExportPrep` 一律补全三列（面板里没有名称入口，所以只有
  "本来就有名称"的列表才带名称列，别把从文件读进来的名字丢了）；纯指令行载体放不下这三项且
  确实有非默认参数时，升级成 Markdown 表格并提示（`qcmdItemHasParams`）。导出的是副本，
  补列只动副本、不碰用户的挂载文件。
