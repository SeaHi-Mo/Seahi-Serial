...
| `.qcmd-group` / `.qcmd-group-hd` | **循环组**：组盒子（折叠时藏掉列标题与数据行）+ 抬头（**拖动握把（最左）** · **组名输入框** · 折叠箭头 · 条数 · 删组；`＋ 添加` 在该组表头行最右）。抬头 `padding-left:22px`、底色 `--surface-3`（从面板左缘铺开，不存在"割裂"）；拖动时 `.dragging` 整条点亮 | flex |
...
- **循环组**（2026-09 加）：一个监视器可以有多组，**每组自成一张表**（抬头 → 列标题 → 数据行）：
  - 抬头依次是：**拖动握把（最左）** · **组名（可重命名）** · 折叠 · 条数 · 删组；
