...
│       └── SKILL.md                 # AI 开发技能指南
├── src/                             # 前端源码（无框架/无构建步骤）
│   ├── index.html                   # 骨架：head + body + 4 个 <link> + 14 个 <script src>
│   ├── css/                         # 4 块样式（主题变量 / 全局 / 快速指令 / 杂项）
...
- **无构建步骤**: 前端按功能拆成 `src/index.html`（骨架）+ `src/css/*.css` + `src/js/*.js`，
  用普通 `<link>` / `<script src>` 直接加载 —— 没有打包器、没有转译，`frontendDist` 就是 `../src`。
  目录结构与加载顺序见 [FRONTEND_LAYOUT.md](FRONTEND_LAYOUT.md)
  （2026-09 之前是一个约 15900 行的单文件 `src/index.html`，拆分是纯搬家、内容一字未改）
...
| `.qcmd-group` / `.qcmd-group-hd` | **循环组**：组盒子（折叠时藏掉列标题与数据行）+ 抬头（**拖动握把（最左）** · **参与开关** · **组名输入框** · 折叠箭头 · 条数 · 删组；`＋ 添加` 在该组表头行最右）。抬头 `padding-left:22px`、底色 `--surface-3`（从面板左缘铺开，不存在"割裂"）；拖动时 `.dragging` 整条点亮 | flex |
...
| `makeQcmdItem(mid, idx, label, value)` | 创建指令条目 DOM（顺序号 / 内容 / 超时 / HEX / 发送 / 删除六格） |
| `addQcmdItem(mid)` | 添加新指令（默认 0 / 3000ms / HEX 关） |
...
| `qcmdItemSeq/Timeout/Expect/Retry/Hex(it)` | 读某条的等待参数（缺省 / 越界 / 脏值都在这里收敛；`timeout` 兼容旧字段 `delay`） |
| `qcmdLoopPlan(mid)` | 参与循环发送的条目（顺序号 > 0）按序号升序 |
| `setQcmdLoop` / `toggleQcmdLoop` | 开/关循环发送（前置不满足则拒绝并说明） |
| `qcmdLoopStep(mid)`（async） | 循环的一步：发当前条 → 等它的回话（arm → 轮询 → 下一步/重发/终止） |
| `qcmdRunOne` / `qcmdWaitVerdict` | 送一条并等它的结论（`_qcmdLoopBusy` 是重入闸：一次只跑一条） |
| `qcmdVerdictOf` / `qcmdStopReasonOf` | Rust 状态 → 前端结论；终止文案（两个纯函数，异步段断言直接测它们） |
| `stopQcmdLoop(mid, reason)` | 停止循环（清定时器/轮询/重入闸 + `qcmd_hs_stop`；`reason` 只在自愈停止时给，会 toast） |
| `qcmdCarryItemPrefs(mid, items)` | 从文件重载时按指令内容带回顺序号/超时/HEX |
...
- 每个监视器独立维护 `quickCmds[]`，每条 = `{ label, value, seq, timeout, expect, retry, hex }`
  （`label` 是外部文件里的「名称」列，**界面上已没有入口**，只原样保留/写回；
  `expect`/`retry` 同样没有面板入口 —— 它们只写在文件表头声明的「期望」「重试」列里，
  但**超时那一格会用它**：配过这两项的条目，那一格描一道淡边、悬停能看到内容）
- 支持动态增删、编辑内容
...
- 宽度可调：**展开后**拖动折叠条即可调宽（往左拖变宽，`cursor:col-resize`）；折叠态不启用拖动（那一下的语义是"展开"）
  - 宽度走 CSS 变量 `--qcmd-side-w`（缺省 300px：一条六格要放得下）；**下限 = 缺省 300px**（用户 2026-09 要求"最小宽度以当前的宽度为准"，六格再窄就挤成一团），**上限跟着窗格走**：
...
- **循环组**（2026-09 加）：一个监视器可以有多组，**每组自成一张表**（抬头 → 列标题 → 数据行）：
  - 抬头依次是：**拖动握把（最左）** · **参与开关** · **组名（可重命名）** · 折叠 · 条数 · 删组；
    组名用 DOM API 造，**不拼 HTML**（组名来自用户文件，可能含引号/尖括号，同 `renderQcmdSource` 的纪律）
  - **参与开关**（握把与组名之间，32×16 滑动开关 + 10px 白圆钮）：关掉 = **这组整组不进循环计划**
    （顺序号原样保留，不动用户文件里的任何格子）。它是组级 UI 状态，跟 `folded` 一样存在 `config.json`
    （用户文件里没有"组级开关"这一列，硬塞会破坏表结构）；关掉的组内容压暗一档。
    ⚠️ 关闭态轨道必须用中性灰（`#525a64`，与「自动映射使能」同一个灰）：用 `--border` 的话
    浅色主题下轨道与白钮都接近白，整个开关会"消失"（用户 2026-09 反馈"开关的白色"）
  - **折叠箭头的两态用颜色区分**（用户 2026-09 要求）：**展开**时箭头是主题色 + 一层淡蓝底（一眼看出这组是展开的），折叠时是灰箭头 + 方向转过去
  ...
  - 每条指令自己的配置（顺序号/超时/期望/重试/HEX/内容）**一律不变**（用户明确要求）：只是被分了段
- **列标题行**（`.qcmd-cols`：顺序 / 指令 / 超时(ms) / HEX）**每组一份**（放在组盒子里），
...
- 每条指令是**一行六格**：`顺序号 | 指令内容 | 超时(ms) | HEX | 发送 | 删除`
  - **顺序号**（方形框，默认 `0`）：`0` = 不参与循环发送；`>0` 参与，**组内**按数字从小到大依次发（同号按列表先后）。参与的那几条**方框蓝底**（`--btn-p` 填充 + 白字，与 HEX 开关的开启态同一套语义）
  - **超时**（默认 `3000`，上限 600000 = 10 分钟）：这条发出去**最多等多久** —— 等到 OK 发下一条、
    等到 ERROR 重发本条、等满还没等到 OK 就终止整条链；清空即回缺省 3000；
    **填 `0` = 这条不等响应**（连续 HEX 帧、设备本来就不回 OK 的指令）
    ⚠️ 这一格以前是「延时」（旧字段 `delay`，语义是"发完隔多久再发下一条"）。语义变了但**照样读得回来**
    （`qcmdItemTimeout` 兜底旧字段），导入带旧列名的文件时提示一次 —— 不做静默语义变更
  - **HEX**（默认关，每条独立）：本条按 HEX 解析后发送；**与主发送栏的文本/HEX 无关** ——
    同一轮循环里"一条文本 + 一条 HEX"是常见需求，跟着全局模式走就做不到
  - 三个控件都带 id（`{mid}-qcmdi-{组id}-{i}-seq/-delay/-hex`）—— 后缀仍叫 `delay`（MCP 的 `domIds`
    与既有断言都按它认门，改名的收益只是"名字好看"，代价是一串连带改动）；
    不带 id 的话 MCP 控件注册表只能靠文档序兜底
- **观感纪律**（2026-09 按"日本排版"那一套收过一版，都是纯视觉、不动交互）：
  - **面（背景）只留两个**：顺序号的方框（用户点名要的顺序标识，底色常驻）与指令内容。
    超时退成"右对齐的一列等宽数字"，平时无面，`hover`/`focus` 才浮出面来 —— 一行里少一个框就安静一档
...
- `seq`/`timeout`/`expect`/`retry`/`hex` 随 `config.json` 持久化（在 `quickCmds` 里）；**循环发送的开关状态不持久化**
  （开机自动发指令太危险），掉线/关监视器/列表里再没有可发条目时**自愈停止**并提示

### 6.4.1 循环发送（`setQcmdLoop` / `qcmdLoopStep`）—— 发一条、等它回话

按顺序号依次发，**每条发完等它的回应**，一轮走完从头再来。节奏由那一条自己的**超时**决定：

```
发一条 ──▶ 等回应 │ busy  ──▶ 继续等（不重发）
                  │ OK    ──▶ 发下一条
                  │ ERROR ──▶ 重发本条（到 retry 上限；用完就终止）
                  └ 等满 timeout 还没等到 OK ──▶ **终止整条链**
```

- 判定**只在 Rust 做一份**（`src-tauri/src/main.rs` 的 `QcmdHs` + 四个命令 `qcmd_hs_arm/state/feed/stop`）：
  - 普通串口：`PortReader` 的读线程收到数据顺手 `hs_feed`（同一块数据，不复制不排队）；
  - WSL：数据只有前端拉得到（Rust 侧没有它的读线程）→ 前端把 `read_wsl_serial` 拉到的块喂给 `qcmd_hs_feed`。
  - ⚠️ **绝不在 JS 里再写一套匹配**（两套必然漂移，而且"两边各自测自己那一半"时都是绿的）
- **arm 必须排在 send 之前**：反过来的话设备的回话可能赶在 arm 之前到达，被当成"上一条的迟到数据"丢掉，
  于是这一条必然白等到超时。前端 `qcmdRunOne` 就是这个顺序
- 跨块：`OK` 完全可能被拆成 `O` + `K\r\n` 两次读上来 —— `QcmdHs` 用累积缓冲**按行**判定
  （`\r\n` / `\n` / `\r` 都算行尾）；`busy` 只算"还在处理"、`ERROR` 优先于 `OK`（`+CME ERROR` 也算失败）、
  整行 `OK` 与 `SEND OK` 这类以 `" OK"` 收尾的都算成功
- 超时**由前端轮询 `qcmd_hs_state` 时结算**（不另起定时器）：没有"定时器忘了清"这种悬挂状态
- 开启前置：串口已连接 **且** 至少有一条顺序号 > 0 —— 不满足当场拒绝并说明原因（绝不"开着但什么都不发"）
- 每步都重取一遍计划（`qcmdLoopPlan`）：中途改顺序号/删条目/改超时/改跳转立刻生效，不用重启循环
- **跳转（分支与循环）**：两个表头列 `成功跳转` / `失败跳转`（面板上没有入口，与「期望」「重试」同一套做法），
  取值 = 留空/`下一条`、**数字（顺序号）**、`结束`。收到 OK 走前者；**ERROR 用尽或超时**都走后者
  （超时也走失败路径是有意的：配网失败多半是超时，只认 ERROR 这功能没用）。
  护栏两条：**连续跳转上限 200**（中间只要顺序走一步就清零 —— 抓的是"只在几条之间打转"的死循环）；
  目标顺序号不存在时**降级**（成功→下一条 / 失败→终止）并把原因写进输出区，绝不静默乱跳
- 发送走的就是 `sendQcmdItem` → `sendQcmdPayload` —— 与手动点发、Enter 键**同一条路径**（不为循环另写一套）
- **重入闸**（`_qcmdLoopBusy[mid]`）：一次只跑一条；每条退出路径都要放闸，否则循环会卡死
- 过程写进**输出区**（`[快速指令] …` 几行：已发出 / 收到 OK → 下一条 / 收到 ERROR → 重发（第 n/N 次）/
  设备回 busy 继续等 / 终止原因）。**面板上没有为它新增任何控件** —— 用户就靠这几行看它在干什么；
  终止时另给一次 `showToast`（写明哪一组第几条、等了多久）
- **折叠起来循环不会停**：分栏折叠后标题行看不见了，所以折叠条上会点一颗一闪一闪的小点
  （`.qcmd-side-tab.loop::after`，仍是主题色 `--link`，不引入新的语义色），`title` 也写明「循环发送进行中，展开可停止」
- 自愈停止的路径：连接掉线（`updateMonitorUI` 里立即停 —— 正在等回话的那一步会被立刻中断，
  **不等它超时**，超时可能被用户设成 10 分钟）、监视器被关闭（`closeMonitor` / `closeWslMonitor`）、
  列表里再没有可发的条目；都带一次 `showToast` 说明原因

### 6.4.2 外部文件（导入 / 导出 / 写回）

> 面向使用者的格式说明（列名别名 / 分组抬头 / 注释与文件头 / 写回规则 / 上限 / FAQ）见 [`QUICK_CMDS.md`](./QUICK_CMDS.md)。本节讲的是**实现与取舍**。
...
- **超时 / 期望 / 重试（+ 顺序号 / HEX）几列：表头驱动**（2026-09）。文件表头写了列名，那一列才被当作该参数：
  - 认得的列名：`名称`（name/label/title）、`指令`（命令/内容/cmd/command/value/data）、
    `顺序号`（顺序/序号/次序/序/order/seq/sequence）、
    **`超时(ms)`**（超时/超时时间/等待/timeout/wait）、
    **`期望`**（期望值/响应词/成功词/expect）、**`重试`**（重试次数/retry/retries）、
    `HEX`（十六进制/格式/format/mode），以及**旧名 `延时`**（延时/延迟/间隔/delay/interval/ms）——
    旧名现在**按超时解读**（语义变了但值照读，导入时提示一次；两列都在时以 `超时` 为准）。
    归一化会抹平大小写、空格与 `(ms)` 这类括注。
    别名表**刻意收窄**：`编号`/`no`/`num`/`index` 这种很可能是用户自己的 ID 列**不认** —— 认错就把 `A1` 改写成 `0`。
  - **「期望」「重试」面板上没有入口**（有些设备回 `WIFI GOT IP` 而不是 `OK`、有些指令要重发更多次）：
    值只从文件读、只写回文件；面板侧的可发现性靠**超时那一格**（配过就描一道淡边 + 悬停列出内容）
    与导入时的一次提示 —— 不做"用户看不见的东西在生效"。
  ...
  - **写回挂载文件不擅自补列**（用户的表结构由用户定）；表头声明了列却缺列的条目，
    新增数据行会按表头的列补齐（`0 / 3000 / false`），不会把表格撑歪。
  - **没动过的格子原样回吐**：用户写的 `0` / `是` / 空格原样保留，只有他真改了值（或面板新建/导出）
    才规范化成 `0 / 3000 / false`（`qcmdParamCell`）—— 往返保真因此仍成立。
  - 文件没声明那几列时，参数只随 `config.json` 走，重载/导入时按**指令内容**带回来
    （`qcmdCarryItemPrefs`）：内容改过就对不上、回到默认；导入这类文件时会**提示一次**
    「这几项只保存在本机配置里，点导出可得到带这些列的文件」（不做静默 no-op）。
  - 顺带：名称已退出界面。**整行都空的条目**不写回文件（写进去也活不过一次重载 —— 解析端把空行当结构行丢掉）；
  但**表头声明了 顺序号/超时/HEX 的文件里，"还没填内容、参数格有值"的行必须留着** ——
  跳掉它，写回/导出的条数就跟面板对不上了（用户 2026-09 报的"文件与前端对不上"）。
- **导出 = 自包含快照**：`qcmdExportPrep` 一律补全 `顺序号 / 指令 / 超时(ms) / HEX / 期望 / 重试` 六列
  （后两项不导出就等于"另存一份把自定义条件丢了"），且**一条都不跳**
  （`fill:true` 把 0 / 3000 / true|false 写全，空条目也占一行）—— 行数必须与面板上的条数一致；
  **不带「名称」列**（面板里没有名称入口，导出的副本就该与面板一一对应；挂载文件自己的名称列
  由写回路径原样保留，不受影响）。纯指令行载体放不下这几项且确实有非默认参数时，
