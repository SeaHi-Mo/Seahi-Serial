...
| # | 位置 | 改什么 | 为什么 |
|---|---|---|---|
| 1 | `renderBleDetail`（`src/js/81-ble.js`） | 现在是 广播内容 / GATT / 数据日志 三块 → 加第 4 块或**独立弹窗**（挂在 `.ble-detail` 的**兄弟**位置上，别放它内部 —— 那个函数会整块重写 innerHTML） | 升级是低频高危操作，弹窗比常驻区块更不容易误触。⚠️ **实现用的是 `.ble-modal-mask` + `.ble-modal` 那套通用弹窗类**；原先这里写"照 `#bleWriteModal`"—— 那个 id 自 **2026-09** 起已不存在（BLE 写入窗改成了设备详情右侧的**侧栏面板** `#bleWritePanel`，见 `doc/FRONTEND_LAYOUT.md` 与 `02-global.css`），所以别再照它抄弹窗结构 |
