...
| `ble_get_output` | **本次会话**的蓝牙数据日志（收到的通知/读回的内容、发出的写）；要跨会话历史用 `channels.rx` 去 `log_tail`。**CTS 的时间条目会自动带解读**（`items[].decodedSummary`） | 读 |
...
而 CTS 的特征值本身是 **10 字节二进制**，人肉解很容易错。**两步就够**（第 4 步只在"字节不是从面板拿的"时才需要）：

```
ble_get_services                                  # 1) 服务树里认「Current Time」(0x1805) 与「Current Time」(2A2B)
ble_read   { "char": "0x2a2b" }                   # 2) 点面板那颗读按钮 → 值进日志
ble_get_output                                    # 3) 条目里直接就有 decodedSummary（一行人话时间）
ble_cts_time { "data": "EA 07 0C 11 0F 2D 3A 04 80 00" }   # 4) 只有手上是"别处拿来的字节"时才需要这一步
```

第 3 步就够了：`ble_get_output` 认得出 `2A2B`/`2A0F` 的特征值，会**就地**给条目补
`decoded`（完整字段）与 `decodedSummary`（一行结论），不用再把这些 HEX 手动喂给 `ble_cts_time`。
判据是**特征短号 + 长度正好对得上** —— 描述符的值不解读（`CCCD` 0x2902 也是 2 字节，
按时间解会给出一个"看着确定、其实完全错"的时区）。`ble_cts_time` 仍然保留：
值的来源不是面板（比如日志里翻出来的一段 HEX）时用它。
