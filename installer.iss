...
/// 下载页面的 OnDownloadProgress 回调
procedure OnDownloadProgress(const Url, FileName: String; const Progress, ProgressMax: Int64);
begin
  if ProgressMax <> 0 then
    DownloadPage.Progress := Round((Progress / ProgressMax) * 100)
  else
    DownloadPage.Progress := 0;
end;
  ...
  try
    ...
    try
      ...
      if not FileExists(MsiPath) then begin
        ...
        if MsgBox(CustomMessage('UsbipdDownloadFailed'), mbErrorInformation, MB_OK) = IDOK then
          Exit;
