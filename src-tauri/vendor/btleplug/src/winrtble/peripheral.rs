...
struct Shared {
    ...
    advertisement_data: RwLock<Option<Vec<u8>>>,
}
...
impl Peripheral {
    pub(crate) fn new(adapter: Weak<AdapterManager<Self>>, address: BDAddr) -> Self {
        ...
        Peripheral {
            shared: Arc::new(Shared {
                ...
                advertisement_data: RwLock::new(None),
            }),
        }
    }
    ...
    fn derive_properties(&self) -> PeripheralProperties {
        PeripheralProperties {
            ...
            advertisement_data: self.shared.advertisement_data.read().unwrap().clone(),
        }
    }
    ...
    pub(crate) fn update_properties(&self, args: &BluetoothLEAdvertisementReceivedEventArgs) {
        ...
        if let Ok(data_sections) = advertisement.DataSections() {
            // 保存原始广告字节：累积合并 Advertising Data 与 Scan Response 的段（按段去重）
            let mut segs: Vec<Vec<u8>> = Vec::new();
            for section in &data_sections {
                if let Ok(dt) = section.DataType() {
                    if let Ok(d) = section.Data() {
                        let payload = utils::to_vec(&d);
                        let mut seg = Vec::with_capacity(payload.len() + 2);
                        seg.push((payload.len() + 1) as u8); // AD len (type + payload)
                        seg.push(dt);                        // AD type
                        seg.extend_from_slice(&payload);     // payload
                        segs.push(seg);
                    }
                }
            }
            if !segs.is_empty() {
                ...
                let mut acc = guard.clone().unwrap_or_default();
                for seg in segs {
                    let dup = !seg.is_empty() && acc.windows(seg.len()).any(|w| w == seg.as_slice());
                    if !dup {
